...
export const parsearCSVAction = panelAction(
  "miembros.importar_previsualizar",
  {},
  async (
    tenant,
    formData: FormData
...
const IMPORT_VACIO: ImportResult = {
  ok: false,
  totalProcessed: 0,
  successCount: 0,
  sinPlanCount: 0,
...
export const importarMiembrosAction = panelAction(
  "miembros.importar",
  {
    onDenied: (d: Denegado): ImportResult => ({
      ...IMPORT_VACIO,
      errors: [{ row: 0, field: "", value: "", reason: d.error }],
    }),
  },
  async (tenant, rows: CSVRow[]): Promise<ImportResult> => {
    if (!rows.length) return IMPORT_VACIO;
