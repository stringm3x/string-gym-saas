...
// Vender productos es inventario (Pro) + permiso vender_desde_caja; se
// verifica en el cuerpo porque la misma acción cobra membresías y visitas,
// que son caja básica.
const PRODUCTOS_SIN_PLAN = "La venta de productos requiere el plan Pro.";
const PRODUCTOS_SIN_PERMISO = "No tienes permiso para vender productos.";

export const registerPagoAction = panelAction(
  "caja.cobrar",
  { onDenied: denegarPago },
  async (tenant, _prev: PagoResult, formData: FormData): Promise<PagoResult> => {
    ...
    if (parsed.data.concepto === "producto") {
      if (!tenant.has("inventario")) {
        return { ok: false, error: PRODUCTOS_SIN_PLAN, fieldErrors: {} };
      }
      if (!tenant.can("vender_desde_caja")) {
        return { ok: false, error: PRODUCTOS_SIN_PERMISO, fieldErrors: {} };
      }
    }
...
export const registrarVisitaRapidaAction = panelAction(
  "caja.visita_rapida",
  { onDenied: denegarPago },
  async (tenant, _prev: PagoResult, formData: FormData): Promise<PagoResult> => {
    const raw = {
 ...
 */
export const registrarAbonoAction = panelAction(
  "caja.abonar_plan_pago",
  {},
  async (
    tenant,
    miembroId: string,
...
export const anularPagoAction = panelAction(
  "caja.anular_pago",
  {},
  async (tenant, pagoId: string, motivo?: string): Promise<{ ok: boolean; error?: string }> => {
...
export const reembolsarPagoAction = panelAction(
  "caja.reembolsar",
  {},
  async (
    tenant,
 ...
 */
export const registrarTicketAction = panelAction(
  "caja.ticket",
  {},
  async (
    tenant,
    ...
    if (input.productos.length > 0) {
      if (!tenant.has("inventario")) return { ok: false, error: PRODUCTOS_SIN_PLAN };
      if (!tenant.can("vender_desde_caja")) {
        return { ok: false, error: PRODUCTOS_SIN_PERMISO };
      }
    }
...
export const getCreditoDisponibleAction = panelAction(
  "caja.credito_disponible",
  { onDenied: () => 0 },
  async (tenant, miembroId: string): Promise<number> => {
    if (!miembroId) return 0;
