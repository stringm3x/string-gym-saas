...
// Anónimas (propósito aceptar_invitacion): el invitado todavía no es staff
// activo de ningún gym, así que no hay rol ni plan que autorizar. La
// identidad se prueba con la sesión que abrió el link de invitación, y la
// pertenencia con staff.user_id === user.id en el cuerpo.
 ...
 */
export const checkInviteStatusAction = anonAction(
  "aceptar_invitacion",
  async (staffId: string): Promise<InviteStatus> => {
 ...
 */
export const acceptInviteAction = anonAction(
  "aceptar_invitacion",
  async (staffId: string, nombre: string, password: string): Promise<AcceptInviteResult> => {
