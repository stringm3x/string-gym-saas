...
| **9b** | Resto de páginas del panel a `requirePanel` con la política de su acción; `role === "owner"` literales fuera de los gates; `pagina.*` para las páginas sin acción propia. | Entregado: 34 páginas/layouts con `requirePanel`; las 8 que no lo usan están listadas con razón en `lib/authz/paginas.test.ts` (agregar una nueva exige tocar la lista). El inventario (2026-09-20) había encontrado 7 divergencias página/acción, no solo `/caja`; todas cerradas. Cambios visibles: `caja/cortes`, `cuentas-por-cobrar` y `onboarding` ya no se ven sin el permiso de su acción; `opiniones` pide `ver_dashboard_ingresos` (paridad con el sidebar); importar CSV y regenerar QR se ven para el gerente. |
